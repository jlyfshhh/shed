...
export async function GET(request: Request) {
  ...
  const [animals, enclosures, careSchedules, careTasks, events, eventRevisions, notes, equipment, weights, feederInventory, feedingAssignments, householdMembers] = await Promise.all([
  ...
  const bundle = { exportedAt: new Date().toISOString(), schemaVersion: 8, animals: animals.results, enclosures: enclosures.results, careSchedules: careSchedules.results, careTasks: careTasks.results, husbandryEvents: events.results, husbandryEventRevisions: eventRevisions.results, animalNotes: notes.results, equipment: equipment.results, weightEvents: weights.results, feederInventory: feederInventory.results, feedingAssignments: feedingAssignments.results, householdMembers: householdMembers.results };
