...
export async function GET(request: Request) {
  ...
  const [animals, careTasks, events, weights, voiceAuditLogs] = await Promise.all([
  ...
  const bundle = { exportedAt: new Date().toISOString(), schemaVersion: 3, animals: animals.results, careTasks: careTasks.results, husbandryEvents: events.results, weightEvents: weights.results, voiceAuditLogs: voiceAuditLogs.results };
