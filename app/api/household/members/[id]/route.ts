...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const payload = await request.json() as { displayName?: string; active?: boolean; reissueAccessCode?: boolean };
    ...
    await db.prepare(
      "UPDATE household_members SET display_name = COALESCE(?, display_name), active = COALESCE(?, active), access_code_hash = COALESCE(?, access_code_hash), updated_at = ? WHERE id = ?",
    ).bind(displayName ?? null, payload.active === undefined ? null : Number(payload.active), accessCodeHash, now, id).run();
    const member = await db.prepare(
      "SELECT id, display_name AS displayName, role, active, created_at AS createdAt, updated_at AS updatedAt, last_login_at AS lastLoginAt FROM household_members WHERE id = ?",
    ).bind(id).first();
    return Response.json({ member: { ...member, active: Boolean((member as { active?: number } | null)?.active) }, accessCode }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
