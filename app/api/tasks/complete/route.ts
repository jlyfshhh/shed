...
export async function POST(request: Request) {
  try {
    ...
    const task = await db.prepare(
      "SELECT id, animal_id AS animalId, task_type AS taskType, title, details FROM care_tasks WHERE id=? AND due_date=?",
    ).bind(payload.taskId, payload.dueDate).first<{ id: string; animalId: string; taskType: string; title: string; details: string }>();
    ...
    if (existing?.voidedAt) {
      await db.prepare(
        "UPDATE husbandry_events SET animal_id = ?, task_type = ?, title = ?, notes = ?, occurred_at = ?, actor_role = ?, completed_by_member_id = ?, completed_by_name = ?, voided_at = NULL, voided_by_member_id = NULL, voided_by_name = NULL, void_reason = NULL WHERE id = ?",
      ).bind(task.animalId, task.taskType, task.title, task.details, occurredAt, actorRole, member?.id ?? null, member?.displayName ?? null, existing.id).run();
    } else if (!existing) {
      await db.prepare(
        "INSERT INTO husbandry_events (id, task_id, animal_id, task_type, title, notes, due_date, occurred_at, actor_role, completed_by_member_id, completed_by_name) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
      ).bind(crypto.randomUUID(), task.id, task.animalId, task.taskType, task.title, task.details, payload.dueDate, occurredAt, actorRole, member?.id ?? null, member?.displayName ?? null).run();
    }

    const completion = await db.prepare(
      "SELECT e.id, e.completed_by_member_id AS completedByMemberId, COALESCE(e.completed_by_name, e.actor_role) AS completedBy, e.occurred_at AS occurredAt FROM husbandry_events e WHERE e.task_id = ? AND e.due_date = ? AND e.voided_at IS NULL",
    ).bind(task.id, payload.dueDate).first();
    return Response.json({ saved: true, completion }, { headers: noStore });
  } catch (error) {
