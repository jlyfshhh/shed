...
# Last, so nothing scrolls it away, and printed on every install rather than
# only the first. Telling people where the file was did not work: it is hidden,
# a file manager will not list it, and someone who has just lost their way in
# needs the value itself, not a lesson in dotfiles.
keeper_key="$(env_value SHED_BOOTSTRAP_TOKEN "$install_dir/.env")"
echo
echo "────────────────────────────────────────────────────────────"
if [ -n "$keeper_key" ]; then
  echo "  Head Keeper key:  $keeper_key"
  echo
  echo "  Use this the first time you open Shed, and any time you need to get"
  echo "  back in. Kept in $install_dir/.env — find it again with:"
  echo "    grep SHED_BOOTSTRAP_TOKEN $install_dir/.env"
else
  echo "  No Head Keeper key is set for this install."
  echo "  Add SHED_BOOTSTRAP_TOKEN to $install_dir/.env and run this again."
fi
echo "────────────────────────────────────────────────────────────"
