...
export async function ensureDatabase(targetDate?: string) {
  ...
  await db.batch([
    ...
    db.prepare("CREATE TABLE IF NOT EXISTS lighting_plans (id TEXT PRIMARY KEY, enclosure_id TEXT NOT NULL, name TEXT NOT NULL, species TEXT, source_name TEXT NOT NULL DEFAULT 'Light My Reptile', source_url TEXT NOT NULL DEFAULT 'https://lightmyreptile.com/', source_version TEXT, planned_on TEXT NOT NULL, reviewed_on TEXT, mounting_mode TEXT, mesh_loss_percent REAL, basking_height REAL, height_unit TEXT NOT NULL DEFAULT 'cm', target_uvi_min REAL, target_uvi_max REAL, target_lux_min REAL, target_lux_max REAL, target_power_density_min REAL, target_power_density_max REAL, plan_sheet_key TEXT, plan_sheet_name TEXT, plan_sheet_type TEXT, notes TEXT, active INTEGER NOT NULL DEFAULT 1, created_at TEXT NOT NULL, updated_at TEXT NOT NULL)"),
    db.prepare("CREATE INDEX IF NOT EXISTS lighting_plans_enclosure_active_idx ON lighting_plans(enclosure_id, active)"),
    db.prepare("CREATE TABLE IF NOT EXISTS lighting_plan_fixtures (id TEXT PRIMARY KEY, plan_id TEXT NOT NULL, equipment_id TEXT NOT NULL, role TEXT NOT NULL, position_cm REAL, mounting_height_cm REAL, quantity INTEGER NOT NULL DEFAULT 1, notes TEXT, created_at TEXT NOT NULL, updated_at TEXT NOT NULL)"),
